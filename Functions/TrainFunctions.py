...
from Functions.SystemIO import save, load
...
def SAENoveltyTrainFunction(data=None, trgt=None, inovelty=0, ifold=0, n_folds=2, n_neurons=1, layer=1,
                            hidden_neurons=[400],
...
def SAEClassifierNoveltyTrainFunction(data=None, trgt=None, inovelty=0, ifold=0, n_folds=2, hidden_neurons=[400],
    ...
    if not os.path.exists(file_name):
        ...
        for i_init in range(n_inits):
            ...
            model.add(
                Dense(hidden_neurons[0], input_dim=norm_data.shape[1], weights=encoder_first_layer, trainable=False))
            model.add(Activation(trn_params.params['hidden_activation']))
