...
# SonarAnalysis/LofarAnalysis Package Setup Script
#
# Author: pedrohblisboa@gmail.com
# Adapted from natmourajr setup scripts
...
export PACKAGE_NAME=$OUTPUTDATAPATH/LofarAnalysis

# Folder Configuration
if [ -d "$OUTPUTDATAPATH/LofarAnalysis" ]; then
    read -e -p "Folder $OUTPUTDATAPATH/LofarAnalysis exist, Do you want to erase it? [Y,n] " yn_erase
    if [ "$yn_erase" = "Y" ]; then
    	echo
...
#        rm -rf $PACKAGE_NAME
#        mkdir $PACKAGE_NAME
#        cd $SONAR_WORKSPACE/Packages/LofarAnalysis
#        for i in $(ls -d */); do 
...
#    	done
    ...
    echo "creating OUTPUTDATAPATH/LofarAnalysis struct"
    echo
    rm -rf $PACKAGE_NAME
    mkdir $PACKAGE_NAME
    cd $SONAR_WORKSPACE/Packages/LofarAnalysis
