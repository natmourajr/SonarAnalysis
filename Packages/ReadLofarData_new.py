...
if os.path.exists("%s/%s/lofar_data_class%i_%i_fft_pts_%i_decimation_rate.mat"%(outputpath,subfolder,1,n_pts_fft,decimation_rate)):
    ...
    for iship, ship in enumerate(ships):
        ...
        print 'Load file: '+"lofar_data_class%i_%i_fft_pts_%i_decimation_rate.mat"%(iship+1,n_pts_fft,decimation_rate)
        matfile = loadmat(
        "%s/%s/lofar_data_class%i_%i_fft_pts_%i_decimation_rate.mat"%
        (outputpath,subfolder,iship+1,n_pts_fft,decimation_rate))
        
        for irun in range(matfile['data_lofar'][0,0]['run'].shape[1]):
			print 'Processing Run: '+str(irun)
			data[iship][irun] = {}
			run = {}
			run['Fs'] = matfile['Fs'][0,0]
			[F, T] = matfile['data_lofar'][0,0]['run'][0,irun].shape
			run['Freq'] = np.linspace(0, run['Fs']/2.0, F)
			run['Time'] = np.linspace(0, T * 1./run['Fs'], T)
			run['Signal'] = matfile['data_lofar'][0,0]['run'][0,irun]
			run['Windows'] = matfile['data_lofar'][0,0]['windonazed_run'][0,irun]
